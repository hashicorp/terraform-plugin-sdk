...
`WriteOnly` should be used for attributes that handle secret values that do not need to be persisted in Terraform plan or state,
such as passwords, API keys, etc. Write-only attribute values are not sent to Terraform and do not
persist in the Terraform plan or state artifacts.
Attributes marked
as `WriteOnly` can accept [ephemeral values](https://developer.hashicorp.com/terraform/language/resources/ephemeral).
